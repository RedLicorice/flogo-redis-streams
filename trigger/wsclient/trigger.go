...
func (*Factory) New(config *trigger.Config) (trigger.Trigger, error) {
	...
	s.SubscribeRequest, err = coerce.ToString(config.Settings["subscribeRequest"])
...
func (t *Trigger) Start() error {
	if t.wsconn != nil {
		go func() {
			...
			if t.settings.SubscribeRequest != "" {
				t.logger.Info("Sending subscribe request")
				t.wsconn.WriteMessage(websocket.TextMessage, []byte(t.settings.SubscribeRequest))
			}
			for {
